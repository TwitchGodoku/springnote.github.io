...
configDefault = {
	...
	consRealSubsOnly	: true,						/* 실제 구독자만 구독콘을 쓸 수 있게 할지
																					 로딩이 빨라짐															*/
	loadDcCons				: true,						// 디시콘을 불러올지
	dcConsURI					: "",								/* 불러올 디시콘 Uri.
																					 로컬 디시콘을 이용할 경우 공백으로 둔다.		*/
  subMonthsMsg			: "",
  ...
  cheersMsg         : "",
	...
	channel						: "#byeolu",			/* 접속할 채널
																					 "#id1,#id2,.."으로 여러 채널에 접속 가능		*/
	retryInterval			: 3,								// 접속에 끊겼을 때 재접속 시도 간격(초)
	allMessageHandle	: true,						// IRC로부터 받은 처리되지 않은 메세지를 html에 표시
...
var checkComplete = function() {
	/* CSS, 디시콘, 이모티콘+구독콘, 후원아이콘, 설정, 그리고 서버 */
	...
	if (++completeCount == num) {
		var chat = addChatMessage("",
			...
			"color:white; background:black; font: normal normal 9pt GulimChe!important; letter-spacing: 0!important;"  + 
		...
		if (chat != null) {
			chat.upper.style.display = "block";
...
debugLog = function(dat) {};
{
	...
	if (configData.debugLevel != 0) {
		if (configData.debugLevel == 1) { debugLog = function(dat) { console.log(dat); }; }
			...
			debugLog = function(dat, unConcat) {
				if (unConcat) {
					addChatMessage("Bridge BBCC", dat,
						{ badges:["admin/1"], color:"red", escape:false });
				}
				else {
					concatChatMessage("Bridge BBCC", dat,
						{ badges:["admin/1"], color:"red", escape:false });
				}
			};
		}
	...
	// debugLog(configLoadMessage + "설정을 불러오고 ");
	checkComplete();
}
...
if (configData.loadTwitchCons) {
	...
	if (configData.consRealSubsOnly) {
		...
		// debugLog("트위치 이모티콘(공통·구독)을 불러오는 중…");
		checkComplete();
	}
	else {
...
if (configData.loadDcCons) {
	...
	dcConScript.onload = function() {
		if (dcConsData.length == 0) { debugLog("디시콘을 불러오는 데 실패했습니다."); }
		else {
			var keywords = [];
			...
			// debugLog("디시콘을 불러오는 중…");
			checkComplete();
		}
	};
}
else {
	debugLog("설정에 따라 디시콘을 적용하지 않았습니다.");
	checkComplete();
}
...
var client = (function() {
							...
							var handler = function(evt) {
								if(clipRequest.readyState == 4) {
									if(clipRequest.status == 200) {
										...
										data["clip"] = '<div class="chat_clip_box invalid"><누군가의 클립></div>';
	...
	ws.onclose = function() {
		debugLog(
			...
			configData.retryInterval + "초 후 접속을 다시 시도합니다.");
			setTimeout(
				function() { client(); },
